...
class GetKfiView(APIView):
    ...
    def get(self, request, user_id):
        try:
            ...
            kfis = Kfi.objects.filter(user_id=user_id)
            if not kfis.exists():
                return Response({"error": "No data found for the given user_id."}, status=status.HTTP_404_NOT_FOUND)
            
            data = {kfi.cell_id: kfi.cell_value for kfi in kfis}
